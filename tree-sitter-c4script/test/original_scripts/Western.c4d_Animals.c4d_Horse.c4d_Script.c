...
public func Redefine(idDef)
{
  ...
  if(this()->~IsHorse()) SetGraphics(Format("%d", graphics), this(),idDef);
  return(1);
}
...
private func Activity()
{
  ...
  if(GetComDir() == COMD_Stop())
    if(!GetXDir())
      if(GetAction() S= "Walk" || GetAction() S= "Trot" || GetAction() S= "Gallop")
        SetGait(0);
  ...
  if(!(GetAction() S= "Jump"))
    ResetRiderPos();   
  // Pferd steht?
  if(GetAction() S= "Stand") SetGait(iGait);
...
private func TurnRight()
{
  ...
  if (GetDir() == DIR_Left())
    if (GetAction() eq "Walk" || GetAction() eq "Stand" || GetAction() eq "Pull") 
      SetAction("Turn");
    else if(GetAction() eq "Turn")
      SetPhase(GetActMapVal("Length","Turn",GetID())-GetPhase()-1);
  // Richtung
  SetDir(DIR_Right());
...
public func Death()
{
  ...
  var graphics = iGraphics;
  ChangeDef(ID_Dead());
  SetGraphics(Format("%d", graphics), this(), ID_Dead()); //  (schlägt fehl, wenn keine Zusatzgrafik vorhanden)
  SetAction("Dead");
  return(1);
}
